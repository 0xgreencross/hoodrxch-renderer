...
function buildGenesis(sState){
  ...
  if(tier>0){
    ...
    const gMid = tier>=4 ? GLOW_W1 : GLOW_P1, gDim = tier>=4 ? GLOW_W2 : GLOW_P2;
    ...
    // outward-only glow: dim echo arcs BEYOND the outermost arc (drawn first, behind)
    const maxOff=5+(nArcs-1)*3;
    let dg=arcPath(maxOff+6); if(dg) f+='<path d="'+dg+'" fill="none" stroke="'+gDim+'" stroke-width="4"/>';
    dg=arcPath(maxOff+3); if(dg) f+='<path d="'+dg+'" fill="none" stroke="'+gMid+'" stroke-width="5"/>';
    ...
    if(tier>=6){ // REAPER: the complete ring, white with a pink echo
      ...
      f+='<ellipse cx="'+rcx+'" cy="'+rcy+'" rx="'+(rx+55)+'" ry="'+(ry+55)+'" fill="none" stroke="'+GLOW_W2+'" stroke-width="4"/>';
      f+='<ellipse cx="'+rcx+'" cy="'+rcy+'" rx="'+(rx+28)+'" ry="'+(ry+28)+'" fill="none" stroke="'+GLOW_W1+'" stroke-width="5"/>';
