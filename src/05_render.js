...
function buildGenesis(sState){
  ...
  // BARCODE / NO SIGNAL keep the blessed static field; everything else flows
  const CONV = t.lineW<=6;
  if(!CONV){
    // --- legacy static field (echoes + grouped rows, exactly as blessed) ---
    let e='', e2='';
    for(const li of ePicks) if(linesD[li].d) e+='<path d="'+linesD[li].d+'"/>';
    for(const li of e2Picks) if(linesD[li].d) e2+='<path d="'+linesD[li].d+'"/>';
    if(tier>=6) for(const L of figLines.slice(0,8)) e+='<path d="'+L.d+'"/>';
    if(e) f+='<g fill="none" stroke="'+PINK+'" stroke-width="'+sw+'" transform="translate('+eDx+' '+eDy+')">'+e+'</g>';
    if(e2) f+='<g fill="none" stroke="'+WHITE+'" stroke-width="'+sw+'" transform="translate('+(-eDx)+' '+eDy+')">'+e2+'</g>';
    const groups={};
    ...
    // === THE CONVEYOR (animation pass): the signal flows upward through a
    // stationary hood envelope. One SMIL animate-d path per grid row (fixed
    // "M + 33 l" structure, integer deci-unit keyframes, linear interpolation);
    // last keyframe = the next row's first, so the loop wraps image-identical.
    // Gaps live in the FIELD: stationary black covers + the blessed break runs.
    const stepD = t.lineW===3?10 : t.lineW===4?40 : 20;    // row spacing in px
    const cycD = t.lineW===6? stepD*2 : stepD;             // DUAL loops 2 rows (parity-safe wrap)
    const NKC = cycD>>1;                                   // keyframe every 2px of travel
    const durS = (cycD*75/1000)+'s';                       // 1.5s per 20px of travel
    const topY=960;                                        // last blessed row position (y=96)
    const rowW = (y0d)=> t.lineW===3?2 : t.lineW===4?5 : t.lineW===5?4 : t.lineW===6?(((y0d/20)%2)?6:2) : [3,4,6][t.lineW];
    // standing disturbances anchored to fixed field positions
    ...
    // one keyframe of one row: geometry + peak height, all integer
    const rowKey=(y0d,pD)=>{
    ...
    // all rows × all keyframes
    const rows=[]; for(let y0d=0;y0d<=1000+cycD;y0d+=stepD) rows.push(y0d);
    const vals=rows.map(()=>[]), mh=rows.map(()=>[]);
    for(let j=0;j<=NKC;j++){ const pD=j*2;
      for(let ri=0;ri<rows.length;ri++){ const [d,m]=rowKey(rows[ri],pD); vals[ri].push(d); mh[ri].push(m); } }
    // per-keyframe colour roles (position-keyed so the wrap stays image-identical)
    const colK=rows.map(()=>[]);
    for(let j=0;j<NKC;j++){
      const figIdx=[]; for(let ri=0;ri<rows.length;ri++) if(rows[ri]>=40&&rows[ri]<=topY&&mh[ri][j]>800) figIdx.push(ri);
      const crestSet=new Set(figIdx.slice(0,crestN)), crest6Set=new Set(figIdx.slice(0,6)), figSet=new Set(figIdx);
      for(let ri=0;ri<rows.length;ri++){
        ...
        if(figSet.has(ri)){
          ...
          else if(pinkFigUpper && rows[ri]-j*2<t.cy*10) col='P';
          else if(pinkCrest6 && crest6Set.has(ri)) col='P';
          if(!whiteFig && crestSet.has(ri)) col='W';
        }
        colK[ri].push(col);
      }
    }
    ...
    { let e='', e2='';
      for(const li of ePicks) if(lines[li]) e+='<path d="'+rowKey(lines[li].y*10,0)[0]+'"/>';
      for(const li of e2Picks) if(lines[li]) e2+='<path d="'+rowKey(lines[li].y*10,0)[0]+'"/>';
      if(tier>=6){ let n=0; for(let ri=0;ri<rows.length&&n<8;ri++) if(rows[ri]>=40&&rows[ri]<=topY&&mh[ri][0]>800){ e+='<path d="'+vals[ri][0]+'"/>'; n++; } }
      if(e) f+='<g fill="none" stroke="'+PINK+'" stroke-width="'+sw+'" transform="translate('+eDx+' '+eDy+')">'+e+'</g>';
      if(e2) f+='<g fill="none" stroke="'+WHITE+'" stroke-width="'+sw+'" transform="translate('+(-eDx)+' '+eDy+')">'+e2+'</g>'; }
    // the flowing field
    f+='<g fill="none">';
    for(let ri=0;ri<rows.length;ri++){
      const y0d=rows[ri];
      const w=rowW(y0d);
      const strokeOf=c=> c==='A'?ACID : c==='P'?PINK : WHITE;
      const widthOf=c=> (c==='A'&&tier>=5) ? Math.max(2,w-1) : w;
      const cols=colK[ri];
      ...
      f+='<path stroke="'+strokeOf(cols[0])+'" stroke-width="'+widthOf(cols[0])+'"'+dash+' d="'+vals[ri][0]+'">';
      f+='<animate attributeName="d" values="'+vals[ri].join(';')+'" dur="'+durS+'" calcMode="linear" repeatCount="indefinite"/>';
      if(varies){
        f+='<animate attributeName="stroke" values="'+cols.map(strokeOf).join(';')+'" dur="'+durS+'" calcMode="discrete" repeatCount="indefinite"/>';
        let wv=false; for(let j=1;j<NKC;j++) if(widthOf(cols[j])!==widthOf(cols[0])) wv=true;
        if(wv) f+='<animate attributeName="stroke-width" values="'+cols.map(widthOf).join(';')+'" dur="'+durS+'" calcMode="discrete" repeatCount="indefinite"/>';
      }
      f+='</path>';
    }
    f+='</g>';
    // the blessed break runs as stationary holes (VAPOR dissolves via dasharray instead)
    if(t.tear!==6){
    ...
    // envelope gap covers: sockets, moth holes, split, censor — exact original
    // gap condition replayed in sampling space, painted at its screen extent
    { const gapC=(x,yd)=>{
